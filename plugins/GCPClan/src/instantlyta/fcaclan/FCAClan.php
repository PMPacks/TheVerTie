	    ...
	    return false;
            ...
            if ($event instanceof EntityDamageByEntityEvent) {
                ...
                if ($this->haveClan($damager) && $this->haveClan($entity) && $this->getProfile($damager)->get("clan") === $this->getProfile($entity)->get("clan")) {
                    $event->setCancelled();
                    $damager->sendMessage("⨶§l§c Người này §e" . $entity->getName() . "§c Cùng Clan Với Bạn!");
                    return;
                } elseif ($entity->getHealth() - $event->getFinalDamage() <= 0) {
                    if ($this->haveClan($entity)) {
                        $this->clanAnnounce("⨶§l§e " . $entity->getName() . " §cBị Người Chơi§e " . $damager->getName() . ($this->haveClan($damager) ? " §f[§cClan§e " . $this->getClanName($damager) . "§f]§c " : "") .
                ...
                switch ($args[0]) {
                    ...
                    case "invite":
                        ...
                        if ($profile->get("clan") === false || !$this->canDo($sender, self::ACTION_INVITE)) {
                            $sender->sendMessage("⨶§l§c Bạn Không Có Quyền Mời Người Chơi Khác Vào Clan!");
                            return true;
                        }
                    ...
                    case "kick":
                        ...
                        if (isset($altArgs[0])) switch ($altArgs[0]) {
                            case "yes":
                                if (isset($this->kickPending[$sender->getName()]) && $this->kickPending[$sender->getName()][0] > time()) {
                                    ...
                                    unset($this->kickPending[$sender->getName()]);
                                    $sender->sendMessage($result ? "⨶§l§c Đã Đuổi Người Chơi . " : "Có Lỗi Xảy Ra Trong Quá Trình Đuổi.");
                                    return true;
                                } else {
                        ...
                        if (count($altArgs) <> 1) {
                            $sender->sendMessage("⨶§l§c Sử Dụng:§e /clan kick <Tên>");
                            return true;
                        }
                        ...
                        if ($found === null) {
                            $sender->sendMessage("⨶§l§c Không Tìm Thấy Người Chơi Nào Với Từ Khóa§e '$needle' §cTrong Clan Của Bạn!.");
                            return true;
                        }
                    ...
                    case "rl":
                        ...
                        if (isset($altArgs[0])) {
                            ...
                            if (($found = $this->getPlayerInRequestList($altArgs[0], $profile->get("clan"))) !== null) {
                                ...
                                $sender->sendMessage("⨶§l§c Không Tìm Thấy Người Chơi Này!");
                            }
                            return true;
                        }
                        ...
                        $mes .= "\n⨞§l§a Gõ§e /clan requestlist <Tên>§a Để Chấp Nhận Người Chơi Vào Clan.";
                        $sender->sendMessage($mes);
                        break;
                    case "delete":
                        if (!($sender instanceof Player)) {
                        ...
                        if (isset($altArgs[0])) {
                            if (isset($altArgs[1])) {
                                ...
                                if (($found = $this->getPlayerInClan($altArgs[0], $profile->get("clan"), $sender->getName())) === null) {
                                    $sender->sendMessage("⨶§l§c Người Chơi Không Có Trong Clan");
                                    return true;
                                }
                    ...
                    case "lookup":
                        if (count($altArgs) <> 1) {
                            $sender->sendMessage("⨶§l§c Sử Dụng:§e /clan lookup <Tên>");
                            return true;
                        }
                        $sender->sendMessage("⨶§l§c Người Chơi " . $altArgs[0] . " " . ($this->haveClan($altArgs[0]) ? "Thuộc Clan " . $this->getClanName($altArgs[0]) : "Không Thuộc Clan Nào."));
                        break;
                    case "reload":
                        ...
                        break;
                    case "top":
                        $mes = "§e❖§l§c Xếp Hạng §e10 Clan§c Xuất Sắc Nhất Máy Chủ §eTheVerTie §e❖\n";
                        for ($i = 0; $i < 10; $i++) {
                    ...
                    case "levelup":
                        ...
                        $sender->sendMessage("⨶§l§c Điểm Clan Có Thể Kiếm Được Bằng Cách Giết Thành Viên Clan Khác(+2) Hoặc Người Chơi Bình Thường(+1)");
