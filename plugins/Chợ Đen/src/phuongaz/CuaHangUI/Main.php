...
Class Main extends PluginBase implements Listener{
    ...
    public function openTradeForm($player)
    {
        ...
        $form->setTitle("§l§e•§c Chợ Đen §e•");
        $form->setContent("§l§6•§e Tiền của bạn: §f".$money);
      ...
      $inv = $player->getInventory();
        if (($this->getTrade($id)['support']['money']) == "true")
		{
			...
			if((EconomyAPI::getInstance()->myMoney($player)) >= $money){
				...
				$player->sendMessage("§l§c§a Bạn đã mua vật phẩm này thành công!");
				$inv->addItem($new);
			}else{
			$player->sendMessage("§l§c Bạn không đủ tiền để mua vật phẩm này");
			}
		}
		if (($this->getTrade($id)['support']['coin']) == "true")
		{
			$coin = $this->getTrade($id)['price']['coin'];
			if((CoinAPI::getInstance()->myCoin($player)) >= $coin){
				$newitem = $this->getTrade($id)['New-Item']['Id'];
				$exn = explode(':', $newitem);
				$idn = $exn[0];
				$metan = $exn[1];
				...
				CoinAPI::getInstance()->reduceCoin($player, $coin);
				$inv->addItem($new);
			}else{
			$player->sendMessage(self::PREFIX. TF::RED. " Bạn không đủ c để mua vật phẩm này");
			}
		}
