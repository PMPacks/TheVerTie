...
class BlazinVanish extends PluginBase{
	...
	public function onEnable() : void{
		...
		$this->getLogger()->info("BlazinVanish " . self::VERSION . " by PIG has been enabled");
	}
	...
	public function onCommand(CommandSender $sender, Command $command, string $label, array $args) : bool{
		if($command->getName() === "vanish"){
			...
			if(!$sender->hasPermission("vanish.command")){
				$sender->sendMessage(self::PREFIX . TextFormat::RED . "§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§c Bạn không có quyền sử dụng lệnh này.");
				return false;
			}
			if(empty($args[0])){
			...
			if($this->getServer()->getPlayer($args[0])){
				...
				if(!in_array($player->getName(), $this->vanish)){
					...
					$sender->sendMessage(self::PREFIX . TextFormat::GREEN . "§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§b Đã bật tàn hình cho§e " . TextFormat::AQUA . $player->getName());
					...
					$sender->sendMessage(self::PREFIX . TextFormat::GREEN . "§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】 §cĐã tắt tàn hình cho §e" . TextFormat::AQUA . $player->getName());
				}
			}else{
				$sender->sendMessage(self::PREFIX . TextFormat::RED . "§l§c【 §fvιcтoʀʏ §eoғ §6ʟᴇԍᴇɴᴅ §c】§c Người chơi không trực tuyến");
				return false;
			}
		}
		return true;
